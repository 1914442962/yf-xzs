...
var Transform = class _Transform {
  ...
  decompose() {
    ...
    if (a != 0 || b != 0) {
      ...
      const s2 = Math.sqrt(c2 * c2 + d * d);
      result.rotation = Math.PI / 2 - (d > 0 ? Math.acos(-c2 / s2) : -Math.acos(c2 / s2));
...
var Node = class _Node {
  ...
  _getTransform() {
    ...
    m.reset();
    const x2 = this.x(), y2 = this.y(), rotation = Konva.getAngle(this.rotation()), scaleX = (_a = this.attrs.scaleX) !== null && _a !== void 0 ? _a : 1, scaleY = (_b = this.attrs.scaleY) !== null && _b !== void 0 ? _b : 1, skewX = this.attrs.skewX || 0, skewY = this.attrs.skewY || 0, offsetX = this.attrs.offsetX || 0, offsetY = this.attrs.offsetY || 0;
    if (x2 !== 0 || y2 !== 0) {
      m.translate(x2, y2);
    }
    if (rotation !== 0) {
...
var Easings = {
  BackEaseIn(t, b, c2, d) {
    const s2 = 1.70158;
    return c2 * (t /= d) * t * ((s2 + 1) * t - s2) + b;
  },
  BackEaseOut(t, b, c2, d) {
    const s2 = 1.70158;
    return c2 * ((t = t / d - 1) * t * ((s2 + 1) * t + s2) + 1) + b;
  },
  BackEaseInOut(t, b, c2, d) {
    let s2 = 1.70158;
    if ((t /= d / 2) < 1) {
      return c2 / 2 * (t * t * (((s2 *= 1.525) + 1) * t - s2)) + b;
    }
    return c2 / 2 * ((t -= 2) * t * (((s2 *= 1.525) + 1) * t + s2) + 2) + b;
  },
  ElasticEaseIn(t, b, c2, d, a, p) {
    let s2 = 0;
    if (t === 0) {
      return b;
    }
    ...
    if (!a || a < Math.abs(c2)) {
      ...
      s2 = p / (2 * Math.PI) * Math.asin(c2 / a);
    }
    return -(a * Math.pow(2, 10 * (t -= 1)) * Math.sin((t * d - s2) * (2 * Math.PI) / p)) + b;
  },
  ElasticEaseOut(t, b, c2, d, a, p) {
    let s2 = 0;
    if (t === 0) {
      return b;
    }
    ...
    return a * Math.pow(2, -10 * t) * Math.sin((t * d - s2) * (2 * Math.PI) / p) + c2 + b;
  },
  ElasticEaseInOut(t, b, c2, d, a, p) {
    let s2 = 0;
    if (t === 0) {
      return b;
    }
    ...
    return a * Math.pow(2, -10 * (t -= 1)) * Math.sin((t * d - s2) * (2 * Math.PI) / p) * 0.5 + c2 + b;
  },
  ...
  BounceEaseIn(t, b, c2, d) {
    return c2 - Easings.BounceEaseOut(d - t, 0, c2, d) + b;
  },
  BounceEaseInOut(t, b, c2, d) {
    if (t < d / 2) {
      return Easings.BounceEaseIn(t * 2, 0, c2, d) * 0.5 + b;
    } else {
      return Easings.BounceEaseOut(t * 2 - d, 0, c2, d) * 0.5 + c2 * 0.5 + b;
    }
  },
  EaseIn(t, b, c2, d) {
    return c2 * (t /= d) * t + b;
...
function filterGaussBlurRGBA(imageData, radius) {
  ...
  for (let x2 = 0; x2 < width; x2++) {
    ...
    for (let y2 = 0; y2 < height; y2++) {
      ...
      b_out_sum -= stackIn.b;
      a_out_sum -= stackIn.a;
      p = x2 + ((p = y2 + radiusPlus1) < heightMinus1 ? p : heightMinus1) * width << 2;
...
function remap(fromValue, fromMin, fromMax, toMin, toMax) {
  ...
  let toValue2 = (fromValue - fromMin) / fromRange;
...
var HSL = function(imageData) {
  const data = imageData.data, nPixels = data.length, v = 1, s2 = Math.pow(2, this.saturation()), h2 = Math.abs(this.hue() + 360) % 360, l = this.luminance() * 127;
  const vsu = v * s2 * Math.cos(h2 * Math.PI / 180), vsw = v * s2 * Math.sin(h2 * Math.PI / 180);
...
var HSV = function(imageData) {
  const data = imageData.data, nPixels = data.length, v = Math.pow(2, this.value()), s2 = Math.pow(2, this.saturation()), h2 = Math.abs(this.hue() + 360) % 360;
  const vsu = v * s2 * Math.cos(h2 * Math.PI / 180), vsw = v * s2 * Math.sin(h2 * Math.PI / 180);
  const rr = 0.299 * v + 0.701 * vsu + 0.167 * vsw, rg = 0.587 * v - 0.587 * vsu + 0.33 * vsw, rb = 0.114 * v - 0.114 * vsu - 0.497 * vsw;
  const gr = 0.299 * v - 0.299 * vsu - 0.328 * vsw, gg = 0.587 * v + 0.413 * vsu + 0.035 * vsw, gb = 0.114 * v - 0.114 * vsu + 0.293 * vsw;
  const br = 0.299 * v - 0.3 * vsu + 1.25 * vsw, bg = 0.587 * v - 0.586 * vsu - 1.05 * vsw, bb = 0.114 * v + 0.886 * vsu - 0.2 * vsw;
...
var Kaleidoscope = function(imageData) {
  ...
  let x2, y2, xoff, i, r, g, b, a, srcPos, dstPos;
...
function y(o) {
  if (!lib_default.autoDrawEnabled) {
    const t = o.getLayer() || o.getStage();
    t && t.batchDraw();
  }
}
var T = { key: true, style: true, elm: true, isRootInsert: true };
...
function O(o, t, a, i) {
  ...
  for (let n in t) {
    ...
    !l && (t[n] !== a[n] || i && t[n] !== e?.getAttr(n)) && (r = true, d[n] = t[n]);
  }
...
function R(o, t) {
  ...
  let e = false;
  i.forEach((d, r) => {
    d.getZIndex() !== r && (d.setZIndex(r), e = true);
  }), e && y(t);
}
var G = lib_default.default?.Stage || lib_default.Stage;
var W = defineComponent({
  ...
  setup(o, { attrs: t, slots: a, expose: i }) {
    ...
    if (!e) return;
    const d = reactive({}), r = ref(null), n = new G({
    ...
    return onMounted(() => {
      ...
      n.destroy();
    }), watch(() => o.config, u, { deep: true }), i({
      getStage: f,
      getNode: l
    }), () => h("div", { ref: r, style: t?.style }, a.default?.());
  }
});
var B = ".vue-konva-event";
var D = {
  Group: true,
  Layer: true,
  FastLayer: true,
  ...
  return defineComponent({
    ...
    setup(a, { attrs: i, slots: e, expose: d }) {
      ...
      onMounted(() => {
        ...
        g(), R(r.subTree, l);
      }), watch(() => a.config, g, { deep: true }), d({
        getStage: u,
        getNode: f
      });
      const m = D.hasOwnProperty(o);
      return () => m ? h("template", {}, e.default?.()) : null;
    }
  });
}
function ye(o, t, a) {
  const i = ref(null), e = ref("loading"), d = (n, l, f) => {
    e.value = "loading";
    const u = new Image();
    ...
    url: toValue(o),
    crossorigin: toValue(t),
    referrerPolicy: toValue(a)
  }));
  return watch(
    r,
...
var s = lib_default.default || lib_default;
var Y = c("Arc", s.Arc);
var Z = c("Arrow", s.Arrow);
var z = c("Circle", s.Circle);
var H = c("Ellipse", s.Ellipse);
var q = c("FastLayer", s.FastLayer);
var J = c("Group", s.Group);
var Q = c("Image", s.Image);
var X = c("Label", s.Label);
var ee = c("Layer", s.Layer);
var ne = c("Line", s.Line);
var te = c("Path", s.Path);
var oe = c("Rect", s.Rect);
var re = c("RegularPolygon", s.RegularPolygon);
var ae = c("Ring", s.Ring);
var ce = c("Shape", s.Shape);
var se = c("Sprite", s.Sprite);
var ie = c("Star", s.Star);
var ue = c("Tag", s.Tag);
var le = c("Text", s.Text);
var de = c("TextPath", s.TextPath);
var fe = c("Transformer", s.Transformer);
var ge = c("Wedge", s.Wedge);
