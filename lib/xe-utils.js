...
(function (global, factory) {
  ...
  function slice(array, startIndex, endIndex) {
    ...
    if (array) {
      for (
        startIndex = startIndex || 0, endIndex = endIndex || array.length;
        startIndex < endIndex;
        startIndex++
      ) {
    ...
    if (targetIndex < len) {
      ...
      if (targetIndex >= 0) {
        ...
        if (startIndex < endIndex) {
          for (
            replaceIndex = 0, replaceArray = array.slice(startIndex, endIndex);
            targetIndex < len;
            targetIndex++
          ) {
  ...
  var searchTree = helperCreateTreeFunc(
    function (parent, obj, iterate, context, path, nodes, parseChildren, opts) {
  ...
  function commafy(num, options) {
    ...
    if (num) {
      ...
      return (
        result[0].replace(
          new RegExp('(?=(?!(\\b))(\\d{' + opts.spaceNumber + '})+$)', 'g'),
          opts.separator
        ) + (result[1] ? '.' + result[1] : '')
      );
    }
  ...
  function toFixedString(str, digits) {
    ...
    var rest = digits
      ? [nums[0], '.', padEnd(nums[1] || '', digits, '0')].join('')
      : nums[0];
  ...
  function helperFixedNumber(str, digits) {
    return toValString(toNumber(str)).replace(
  ...
  function toStringDate(str, format) {
    ...
    if (str) {
      ...
      if (isDateType || (!format && /^[0-9]{11,15}$/.test(str))) {
        rest = new Date(
          isDateType ? helperGetDateTime(str) : staticParseInt(str)
        );
      } else if (isString(str)) {
        format = format || setupDefaults.formatDate;
        arrayEach(dateFormatRules, function (item) {
          for (
            index = 0, rules = item.rules, len = rules.length;
            index < len;
            index++
          ) {
            arr = rules[index];
            ...
            if (tempMatch) {
              rest = new Date(
                helperGetUTCDateTime(dates) -
                  (tempMatch[1] === '-' ? -1 : 1) *
                    staticParseInt(tempMatch[2]) *
                    3600000 +
                  staticParseInt(tempMatch[3]) * 60000
              );
            }
          }
        } else {
  ...
  function toDateString(date, format, options) {
    if (date) {
      ...
      if (isDate(date)) {
        ...
        var formats = assign(
          {},
          setupDefaults.formatStringMatchs,
        ...
        var timeRules = [
          [
            ...
            '',
            function (match) {
              return ('' + helperGetDateFullYear(date)).substr(
                4 - match.length
              );
            },
          ],
          ...
          [
            ...
            '',
            function (match) {
              return formatPadStart(
                handleCustomTemplate(date, formats, match, getYearWeek(date)),
                match.length,
                '0'
              );
            },
          ...
          [
            ...
            function (match) {
              return formatPadStart(
                handleCustomTemplate(date, formats, match, getYearDay(date)),
                match.length,
                '0'
              );
            },
      ...
      if (day || !isNaN(day)) {
        if (day === staticStrFirst) {
          return new Date(
            ...
            helperGetDateMonth(date) + monthOffset,
            1
          );
        } else if (day === staticStrLast) {
          return new Date(
            helperGetDateTime(
              getWhatMonth(date, monthOffset + 1, staticStrFirst)
            ) - 1
          );
        } else {
          date.setDate(day);
  ...
  function getYearWeek(date) {
    ...
    if (isDate(date)) {
      ...
      return (
        Math.round(
          ((date.getTime() - week.getTime()) / staticDayTime +
            ((week.getDay() + 6) % 7) -
            3) /
            7
        ) + 1
  ...
  function getMonthWeek(date) {
    ...
    if (isDate(currentDate)) {
      ...
      if (currentDate >= monthFirstWeek) {
        return (
          Math.floor(
            (helperGetYMDTime(currentDate) - helperGetYMDTime(monthFirstWeek)) /
              staticWeekTime
          ) + 1
        );
      }
  ...
  function trimLeft(str) {
    return str && str.trimLeft
      ? str.trimLeft()
      : toValString(str).replace(/^[\s\uFEFF\xA0]+/g, '');
  }
  ...
  function template(str, obj) {
    return toValString(str).replace(
      /\{{2}([.\w[\]\s]+)\}{2}/g,
      function (match, key) {
    ...
    if (isNumber(obj)) {
      if (('' + obj).indexOf('e-') >= 0) {
        ...
        return (
          (isNegative ? '-' : '') +
          '0' +
          ('' + ((isNegative ? Math.abs(obj) : obj) + 1)).substr(1)
        );
      }
    }
  ...
  function stringifyParams(resultVal, resultKey, isArr) {
    ...
    each(resultVal, function (item, key) {
      ...
      if (isPlainObject(item) || _arr) {
        ...
        result.push(
          staticEncodeURIComponent(resultKey + '[' + (isArr ? '' : key) + ']') +
            '=' +
          ...
          if (opts.name) {
            ...
            arrayEach(['expires', 'path', 'domain', 'secure'], function (key) {
              if (!isUndefined(opts[key])) {
                values.push(
                  opts[key] && key === 'secure' ? key : key + '=' + opts[key]
                );
              }
            });
          }
