      ...
      >
        <template #action="{ row }">
          ...
          <vxe-button mode="text" status="primary" @click="doSendMessage(row)"
            >发送短信</vxe-button
          >
          <vxe-button mode="text" status="primary" @click="previewMethod(row)"
            >预览</vxe-button
          >
          <vxe-button mode="text" status="primary" @click="downloadMethod(row)"
            >下载</vxe-button
          >
        </template>
        <template #status="{ row }">
          <span>{{ row.status ? row.status : "未确认" }}</span>
        </template>
      </vxe-grid>
    </article>
  </div>
...
export default {
  ...
  methods: {
    ...
    downloadMethod(row) {
      drfxApi.getFileIdService({ data: row }).then((res) => {
    ...
    previewMethod(row) {
      drfxApi.getFileIdService({ data: row }).then((res) => {
        if (this.$store.state.isXzs) {
          commonApi.preViewFileGetService({ fileId: res.data.id });
        } else {
          commonApi.preViewFileService({ fileId: res.data.id }).then((res) => {
