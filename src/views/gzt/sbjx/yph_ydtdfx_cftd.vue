...
import ycftdApi from "@/api/ycftd-api.js";
import commonApi from "@/api/common-api.js";

export default {
  name: "yph_ydtdfx_cftd",
  mixins: [table_mixin],
  ...
  methods: {
    ...
    loadTableApi(page, sorts, filters) {
      ...
      return new Promise((resolve) => {
        ycftdApi.loadTableDataService(params).then((res) => {
    ...
    importData() {
      ycftdApi.importDataService({}).then((res) => {
        this.$message.success(res.data);
        this.loadTableData();
      });
    },
    createData() {
      const rows = this.getCheckboxRecords();
      if (rows && rows.length > 0) {
        ycftdApi.createFxyjDataService({ data: rows }).then((res) => {
          this.$message.success(res.data);
        });
      }
    },
    ...
    doRealSave(data) {
      ycftdApi.saveTableDataService({ data }).then((res) => {
        this.$message.success(res.data);
