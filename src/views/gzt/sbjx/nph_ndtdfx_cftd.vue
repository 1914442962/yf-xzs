...
import ncftdApi from "@/api/ncftd-api.js";
import dayjs from "dayjs";
import commonApi from "@/api/common-api.js";

export default {
  name: "nph_ndtdfx_cftd",
  mixins: [table_mixin],
  ...
  methods: {
    ...
    loadTableApi(page, sorts, filters) {
      ...
      return new Promise((resolve) => {
        ncftdApi.loadTableDataService(params).then((res) => {
    ...
    importData() {
      ncftdApi.importDataService({}).then((res) => {
        this.$message.success(res.data);
        this.loadTableData();
      });
    },
    createData() {
      const rows = this.getCheckboxRecords();
      if (rows && rows.length > 0) {
        ncftdApi.createFxyjDataService({ data: rows }).then((res) => {
          this.$message.success(res.data);
        });
      }
    },
    ...
    doRealSave(data) {
      ncftdApi.saveTableDataService({ data }).then((res) => {
        this.$message.success(res.data);
